...
MODEL_OUTPUT_SUBFOLDERS = {
    'EvSnowNet_Paper': 'evsnownet_paper',
}
...
def compute_metrics_across_dataset(dataset_folder, groundtruth_folder, groundtruth_subfolder='groundtruth', estimated_subfolder='estimated',
    ...
    Args:
        ...
        visualize: whether to visualize the images being compared
    """
    all_psnr_values = []
    all_ssim_values = []
    ...
    for folder in folders:
        ...
        for gt_img, est_img in zip(gt_images, est_images):
            # print(f'Processing {folder} - {groundtruth_folder}/{groundtruth_subfolder}/{gt_img} vs {estimated_subfolder}/{est_img}')
            img1 = io.imread(os.path.join(gt_path, gt_img))
            img2 = io.imread(os.path.join(est_path, est_img))
            # convert to BGR for consistency with previous metric behavior
            ...
            # visualize the images being compared (optional):
            if visualize:
                # draw text with PSNR on the combined image on the right side
                text = f'PSNR: {psnr_value:.2f} dB'
                cv2.putText(combined, text, (img1.shape[1] + 10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                cv2.imshow('Ground Truth (Left) vs Estimated (Right)', combined)
                cv2.waitKey(10)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate PSNR/SSIM on inference outputs.')
    parser.add_argument('--dataset_folder', type=str, required=True,
                        help='Root folder containing per-sequence result folders.')
    parser.add_argument('--groundtruth_folder', type=str, default=None,
                        help='Root folder containing per-sequence ground truth folders. Defaults to dataset_folder.')
    parser.add_argument('--groundtruth_subfolder', type=str, default='gt',
                        help='Name of subfolder containing ground truth images in each sequence folder.')
    parser.add_argument('--estimated_subfolder', type=str, default=None,
                        help='Explicit estimated subfolder name. If not provided, inferred from --model_name.')
    parser.add_argument('--model_name', type=str, default='EvSnowNet_Paper',
                        help='Model name used to infer estimated output folder when --estimated_subfolder is not set.')
    parser.add_argument('--output_file', type=str, default=None,
                        help='Optional explicit output metrics file path.')
    parser.add_argument('--visualize', action='store_true',
                        help='Whether to visualize the images being compared during evaluation.')
...
if __name__ == "__main__":
    ...
    groundtruth_folder = args.groundtruth_folder or dataset_folder
    groundtruth_subfolder = args.groundtruth_subfolder
    estimated_subfolder = args.estimated_subfolder

    if estimated_subfolder is None:
        if args.model_name not in MODEL_OUTPUT_SUBFOLDERS:
            raise ValueError(
                f"Unknown model_name '{args.model_name}'. Provide --estimated_subfolder explicitly."
            )
        estimated_subfolder = MODEL_OUTPUT_SUBFOLDERS[args.model_name]

    mean_psnr, mean_ssim = compute_metrics_across_dataset(
        dataset_folder,
        groundtruth_folder,
    ...
    if args.output_file is None:
        output_file = os.path.join(dataset_folder, f'{estimated_subfolder}_metrics.txt')
    else:
        output_file = args.output_file

    print(f'Writing metrics to: {output_file}')
    write_metrics_to_file(output_file, mean_psnr, mean_ssim)